    ...
    <div class="mb-8 flex flex-wrap justify-between md:mb-16">
        ...
        <div class="flex flex-col w-full max-w-3xl mx-auto prose text-left prose-blue">
            <div class="w-full mx-auto">
                <h1 class="text-2xl font-bold text-gray-800 md:mb-6 lg:text-3xl">{{ $data['title'] }}</h1>
                <div class="mb-4">
                    @php
                        $date = str_replace('/', '-', $data['created_at']);
                        $newDate = date("M d, Y", strtotime($date));
                    @endphp
                    <p class="mt-3 text-md text-gray-600 dark:text-gray-400">
                        <span>{{ $newDate }}</span>
                    </p>
                </div>
                <div class="my-5 p-3 flex justify-center items-center shadow rounded-lg">
                    <img src="{{ $data['image'] != NULL ? $data['image'] : "https://dummyimage.com/420x260/ddffdd/007300&text=Image+Not+Found"}}" class="h-72" alt="Gambar" srcset=""/>
                </div>
                {!! $data['content'] !!}
                <div class="my-5 text-center">
                <div class="inline-block bg-white shadow-md rounded-2xl py-3 px-4 dark:bg-gray-800 w-full">
                    <div class="flex flex-col items-center gap-x-1.5">
                        ...
                        <div>
                            ...
                            @foreach ($collection as $tags)
                                <p class="mt-3 inline-flex items-center gap-1.5 py-1.5 px-3 rounded-full text-xs font-medium bg-green-100 text-green-500">
                                    {{ $tags }}
                                </p>
                            @endforeach
                        </div>
